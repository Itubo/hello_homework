...
class JStomp {
  ...
  factory JStomp()  {
    return _getInstance()?? JStomp._init();
  }
  ...
  /// 打开stomp连接
  ///
  Future<bool> connection(onOpen,
      {onError,onClosed}) async {
    ///先注册连接状态监听器
  ...
  Future<bool> subscribP2P(List<String> urls) async {
    ...
    return await _channel?.invokeMethod(_NativeMethod.SUBSCRIBER_P2P, urlStr);
  }
  ...
  Future<bool> onMessageCallback(OnMessageCallback onMessage,
    ...
    return await _channel?.invokeMethod(_NativeMethod.MESSAGE_CALLBACK);
  }
  ...
  ///
  Future<String>  sendMessage(String message, {Map<String, dynamic>? header}) async{
    ...
    return await _channel?.invokeMethod(_NativeMethod.SEND_MESSAGE, params);
  }
  ...
  Future<bool> onSendCallback(OnSendMessageCallback callback) async {
    ...
    return await _channel?.invokeMethod(_NativeMethod.SEND_CALLBACK);
  }
  ...
  Future<dynamic> _nativeHandle(MethodCall call) async {
    ...
    switch (method) {
      ...
      ///发送消息回调
        ...
        _sendController?.add(_OnSendMessageData(params["status"], params["msg"]));
        break;
      case _NativeMethod.ON_MESSAGE:

      ///收到新消息
        _messageController?.add(new _OnMessageData(_MessageType.P2P, call.arguments));
        break;
      case _NativeMethod.ON_BROAD_CAST:

      ///接到到新广播消息回调
        _messageController?.add(new _OnMessageData(_MessageType.BROADCAST, call.arguments));
        break;
      case _NativeMethod.ON_CONNECTION_OPENED:

      ///连接打开回调
        _connectionController?.add(_OnConnectionData(_Connection.OPEN, call.arguments));
        break;
      case _NativeMethod.ON_CONNECTION_ERROR:

      ///连接错误回调
        _connectionController?.add(_OnConnectionData(_Connection.ERROR, call.arguments));
        break;
      case _NativeMethod.ON_CONNECTION_CLOSED:

      ///连接断开回调
        _connectionController?.add(_OnConnectionData(_Connection.CLOSED, call.arguments));
        break;
    }
    return Future.value("");
  }
