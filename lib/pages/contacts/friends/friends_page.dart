...
class _FriendsPageState extends State<FriendsPage> {
  ...
  final List<Friends> _headerData = [
    Friends(imageAssets: 'assets/images/新的朋友.png', name: '新的朋友'),
    Friends(imageAssets: 'assets/images/群聊.png', name: '群聊'),
    Friends(imageAssets: 'assets/images/公众号.png', name: '公众号'),
  ];
  ...
  void getListData() async {
    datas.removeRange(0, datas.length);

    Dio dio = Dio();
    var userDetail = {'uid': Global.uid};
    Response response =
        await dio.post(Global.baseURL + "/getAllFriend", data: userDetail);
    ...
    for (var i = 0; i < list.length; i++) {
      ...
      Friends friends = Friends(
          imageUrl: list[i]['avatar'].toString(),
          name: list[i]['username'].toString(),
          indexLetter: list[i]['fA'].toString(),
          uid: list[i]['uid']);
      setState(() {
        print(friends);
        datas.insert(0, friends);
      });
    }
  }
  ...
  @override
  Widget build(BuildContext context) {
    _listDatas.removeRange(0, _listDatas.length);
    _listDatas.addAll(datas); //数据
        ...
        actions: [
          ...
          GestureDetector(
            ...
            child: Container(
              ...
              child: const Image(
                image: AssetImage(
                  "assets/images/icon_friends_add.png",
                ),
                width: 25,
                color: Colors.white,
              ),
              ...
              itemBuilder: (BuildContext context, int index) {
                if (index < _headerData.length) {
                  ...
                  return GestureDetector(
                    ...
                    onTap: () {
                      print("我点击这个元素了！");
                      print(_listDatas[index - _headerData.length].uid);
                      Navigator.of(context).push(MaterialPageRoute(
                          builder: (BuildContext context) => new ShowInfo(
                                uid: _listDatas[index - 3].uid,
                                isF: true,
                              )));
                    },
                    child: _FriendCell(
                        imageUrl: _listDatas[index - 3].imageUrl,
                        name: _listDatas[index - 3].name,
                        groupTitle:
                            isShowT ? null : _listDatas[index - 3].indexLetter),
                  );
                }
              },
    ...
    if (index < _headerData.length) {
      ...
      return GestureDetector(
        ...
        child: _FriendCell(
            imageUrl: _listDatas[index - 3].imageUrl,
            name: _listDatas[index - 3].name,
            groupTitle: isShowT ? null : _listDatas[index - 3].indexLetter),
      );
    }
  }
}
